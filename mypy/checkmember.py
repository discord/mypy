...
def analyze_class_attribute_access(itype: Instance,
    ...
    if isinstance(node.node, TypeInfo):
        ...
        return apply_class_attr_hook(mx, hook, type_object_type(node.node, mx.named_type))

    if isinstance(node.node, MypyFile):
        # Reference to a module object.
        assert False, "TODO RIGHT NOW NOMERGE - No tests hit this, how to trigger?"
        return apply_class_attr_hook(mx, hook, mx.named_type('types.ModuleType'))

    if (isinstance(node.node, TypeAlias) and
            isinstance(get_proper_type(node.node.target), Instance)):
        return apply_class_attr_hook(mx, hook, instance_alias_type(node.node, mx.named_type))
